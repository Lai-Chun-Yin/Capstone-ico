...
export type AuthActions = AuthStartAction | AuthFailAction | AuthLogoutAction | 
AuthSuccessAction|SetAuthRedirectPathAction;
...
export const loginFacebook = (accessToken:string)=>{
    return (dispatch: Dispatch<AuthActions>)=>{
        dispatch(authStart());
        axios.post(
          `${process.env.REACT_APP_API_SERVER}/api/auth/facebook`,
          {
...
export const loginGoogle = (id_token:string)=>{
    return(dispatch: Dispatch<AuthActions>)=>{
        dispatch(authStart());
        axios.post(
            `${process.env.REACT_APP_API_SERVER}/api/auth/google`,
            {id_token}
        ).then((response:any)=>{
            if(!response.data){throw new Error("Unknown error")}
        })
